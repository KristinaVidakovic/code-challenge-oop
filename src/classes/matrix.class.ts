...
export class Matrix {
    ...
    getCharacterAtPosition(pos: Position): string {
        return this.matrix[pos.y]?.[pos.x] ?? ' ';
    }

    findCharacterPosition(char: string): Position | null {
        for (let y = 0; y < this.matrix.length; y++) {
            for (let x = 0; x < this.matrix[y].length; x++) {
                if (this.matrix[y][x] === char) return { x, y };
            }
        }
        return null;
    }

    hasMultipleOccurrences(char: string): boolean {
        let count = 0;
